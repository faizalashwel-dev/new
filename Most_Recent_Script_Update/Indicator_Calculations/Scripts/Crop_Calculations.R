...
length_temp<-length(grep("crop_sold_income_", colnames(dat_all)))
...
for (crops in 1:length(grep("crop_name_", colnames(dat_all))))
{
  ...
  {
    if (!is.na(dat_all[j,colnames(dat_all)==paste0("crop_name_",crops)]))
    {
...
for (i in 1:length_temp)
{
  ...
  for(j in 1:nrow(dat_all))
  {
    ...
    {
      sold_income_units_column[j]<-"total_income_per_year" 
      sold_income_character_temp<-"total_income_per_year" 
    }
  ...
  for(j in 1:nrow(dat_all))
  {
    ...
    if (dat_all[j,colnames(dat_all)==paste0("crop_name_",i)] %in% unique_crops==TRUE)
    {
      if (length(grep("intercrop", intercropping[j]))>0)
      {
        ...
        Crops_Monocropped[j,colnames(Crops_Monocropped)==dat_all[j,colnames(dat_all)==paste0("crop_name_",i)]]<-FALSE
      ...
      if (length(grep("monoculture", intercropping[j]))>0)
      {
        ...
        Crops_Intercropped[j,colnames(Crops_Intercropped)==dat_all[j,colnames(dat_all)==paste0("crop_name_",i)]]<-FALSE
