...
# plot
# ggsave("Word_Outputs/6.Pratiques_Agricole/1.Crops_use.png")
###------------------------------
#### Determining most common crops grown #####
if (sum(dat_all$crops_all=='NA')!=nrow(dat_all))
{
...
crops_most_important<-data.frame("Culture"=row.names(crops_most_important), "Pourcentage des Ménages qui Cultivent"=crops_most_important[,1]) 
crops_most_important<-crops_most_important[order(crops_most_important[,2], decreasing = TRUE),]

if ("other" %in% crops_most_important$Culture)
{
crops_most_important<-crops_most_important[-grep("other", crops_most_important$Culture),]
}
write.csv(crops_most_important, "Word_Outputs/6.Pratiques_Agricole/1.Resume_des_Cultures.csv", row.names = F)


png("Word_Outputs/6.Pratiques_Agricole/1.Graphique_Resume_des_Cultures.png", height = 1000, width = 1200)
par(cex=1, mar=c(10,5,5,5))
barplot(crops_most_important$Pourcentage.des.Ménages.qui.Cultivent, names.arg = crops_most_important$Culture, main = "Tout les Cultures Cultivées", las=2)
dev.off()
}
###------------------------------
#### Crop Residue_Use ####
...
# plot
# ggsave("Word_Outputs/6.Pratiques_Agricole/3.Crops_Residue_Uses.png")

###------------------------------
#### Yield and Harvests ####

imp_crop_yields<- dat_crop[,grep("Yield", colnames(dat_crop))]
colnames(imp_crop_yields)<- gsub("Yield_", "", colnames(imp_crop_yields))
imp_crop_yields<- data.frame(lapply(imp_crop_yields, function (x) as.numeric(as.character(x))))


imp_crop_harvests<- dat_crop[,grep("Harvested", colnames(dat_crop))]
colnames(imp_crop_harvests)<- gsub("Harvested_", "", colnames(imp_crop_harvests))
imp_crop_harvests<- data.frame(lapply(imp_crop_harvests, function (x) as.numeric(as.character(x))))


if(sum(rowSums(!is.na(data.frame(imp_crop_yields))), na.rm = T)!=0)
{
png("Word_Outputs/6.Pratiques_Agricole/2.Rendement_des_Cultures.png", height = 700, width = 700); par(mar=c(9,4,4,2), cex=1.6)
boxplot(data.frame(imp_crop_yields), las=2, col="lightgray", ylab="Rendement des Culture (tons/ha)",  main="Rendement des Culture\n(par hectare)", outline=F)
dev.off()
}


 png("Word_Outputs/6.Pratiques_Agricole/4.Recolte_des_Culture.png", height = 700, width = 700); par(mar=c(9,4,4,2), cex=1.6)
 boxplot(data.frame(imp_crop_harvests), las=2, col="lightgray", ylab="Récolte (tons)",  main="Crop Yields\n(tons)", outline=F)
 dev.off()
...
colnames(crop_yield_df)<- c('Culture', 'Médian_Rendement', 'Gamme_Interquartile_Rendement', 'Médian_Récolte', 'Médian_Récolte')

write.csv(crop_yield_df, "Word_Outputs/6.Pratiques_Agricole/2.Rendement_et_Recolte_des_Cultures.csv", row.names = F)

###------------------------------
if (sum(dat_all$livestock_all=='NA')!=nrow(dat_all))
{
...
colnames(Livestock_most_important)<- c('Bétail Garder', '% des Ménages')
write.csv(Livestock_most_important, "Word_Outputs/6.Pratiques_Agricole/3.Resume_Betail.csv", row.names = F)


png("Word_Outputs/6.Pratiques_Agricole/3.Graphique_Resume_Betail.png", height = 1000, width = 1200)
par(cex=1, mar=c(10,5,5,5))
barplot(Livestock_most_important$`% des Ménages`, names.arg = Livestock_most_important$`Bétail Garder`, main = "Bétail Garder", las=2, ylab = "% des Ménages", ylim=c(0,100))
dev.off()
}
...
png("Word_Outputs/6.Pratiques_Agricole/4.Tetes_de_Betails.png", height = 1000, width = 1200)
boxplot(livestock_heads_temp, main = "Têtes de Bétails", las=2, ylab = "Nombre de Têtes de Bétails",  outline=F)
dev.off()
...
colnames(livestock_heads_summary)<- c("Bétail", "Têtes (Médian)","Têtes (Gamme Interquartile)" )

write.csv(livestock_heads_summary, "Word_Outputs/6.Pratiques_Agricole/4.Tetes_de_Betails.csv", row.names = F)
