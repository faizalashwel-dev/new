...
dat$TVA_quartiles <- ifelse(dat$TVA_pmae_pday<=quarts[2], "inférieure",
                        ifelse(dat$TVA_pmae_pday>quarts[2] & dat$TVA_pmae_pday<=quarts[3], "moyen inférieure", 
                               ifelse(dat$TVA_pmae_pday>quarts[3] & dat$TVA_pmae_pday<=quarts[4], "moyen supérieure",
                                      ifelse(dat$TVA_pmae_pday>quarts[4] & dat$TVA_pmae_pday<=quarts[5], "haute", NA))))
...
colnames(TVA_and_Cash_Income) <- c("Quartiles", 
                                   "Médian Valeur Totale des Activités ($/personne/jour)", 
                                   "Médian Revenu ($/personne/jour)") 
# whole_pop<- c("All", 
...
write.csv(TVA_and_Cash_Income, "Word_Outputs/2.Resume_des_donnees/1.VTA_et_Revenu.csv", row.names = F)
...
colnames(Hungry_Months_and_HDDS_FIES) <- c("Quartiles", 
                                   "Mois de Faim (médian)", 
                                   "Médian Diversité Alimentaire (Bonne Saison)",
                                   "Médian Diversité Alimentaire (Mauvaise Saison)")
                                   #"FIES Status") 
...
write.csv(Hungry_Months_and_HDDS_FIES, "Word_Outputs/2.Resume_des_donnees/2.Mois_de_Faim_et_Securite_Alimentaire.csv", row.names = F)
...
write.csv(Farm_and_Household_Characteristics, "Word_Outputs/2.Resume_des_donnees/3.Carecteristiques_de_la_ferme_et_menages.csv", row.names = F)
...
# 
# 
# write.csv(NRM_Bio_df, "Word_Outputs/2.Resume_des_donnees/4.NRM_Bio.csv", row.names = F)
# write.csv(NRM_Soil_Water, "Word_Outputs/2.Resume_des_donnees/5.NRM_Soil_and_Water.csv", row.names = F)
# write.csv(NRM_Gully, "Word_Outputs/2.Resume_des_donnees/6.NRM_Gully.csv", row.names = F)
# 
...
colnames(crops_most_important)<- c('Cultures', '% de Ménages Cultivant')
colnames(crops_most_important)<- c('Bétails', '% de Ménages')


write.csv(crops_most_important, "Word_Outputs/2.Resume_des_donnees/3.Resume_Culture.csv", row.names = F)
write.csv(Livestock_most_important, "Word_Outputs/2.Resume_des_donnees/4.Resume_Betails.csv", row.names = F)
#write.csv(NTFP_most_important, "Word_Outputs/2.Resume_des_donnees/9.NTFP_Summary.csv", row.names = F)
#write.csv(Food_Trees_most_important, "Word_Outputs/2.Resume_des_donnees/10.Food_Tree_Summary.csv", row.names = F)
