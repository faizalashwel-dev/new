...
dat$TVA_quartiles <- factor(dat$TVA_quartiles, levels=c("inférieure","moyen inférieure","moyen supérieure","haute"))
...
rownames(temp)<-c("Hors d'Exploitation", "Bétails Vendu", "Bétail Consommés", "Cuture Vendu", "Culture Consommés")

write.csv(temp, "Word_Outputs/3.Revenu_et_Productivite_des_Menages/6.Prop_PFA_Table.csv")
...
png("Word_Outputs/3.Revenu_et_Productivite_des_Menages/6.Prop_sources_calorie_potentielle.png")
par(mfrow=c(1,1),mar=c(8,5,4,8), mgp=c(3,3,1))
barplot(100*Contr_FoodCrops_cons, width=Width,cex.axis=1.5,cex.names=1.2,names.arg =c('Inférieure', 'Moyen\nInférieure', 'Moyen\nSupérieure', 'Haute'), col='green',border=TRUE, ylim=c(0,100), main="Apport de Calories (%)" , xlab="")
...
legend(x=1.22, y=75, xpd = T, pch=21, col = c('black','black','black','black','black'),pt.bg= c('black','red','orange','blue','green'),c("Hors d'Exploitation",'Bétails Vendus','Bétails Consommés','Cultures Vendus','Cultures Consommés'),bg='white')
dev.off()
...
png("Word_Outputs/3.Revenu_et_Productivite_des_Menages/3.Disponibilite_Alimentaire.png", height = 800, width = 1000)
par(cex=2)
barplot((as.numeric(dat$FAEnergyCropConsumption))[FSSorder], col="green", ylim=ylim, ylab="Disponibilité Alimentaire [kcal/EHA/jour]", border=NA)
...
legend("topleft",legend=c("Hors d'Exploitation","Bétails Vendus", "Bétails Consommer", "Cultures Vendus", "Cultures Consommer", paste(FS.p, "% Ligne de Calories")), fill=c("black", "orange", "red", "blue", "green",NA))

title(paste0('Disponibilité Alimentaire\n', Country, "_", Year), cex.main=1.5)
dev.off()
...
rownames(temp)<-gsub("Contr_","",rownames(temp))
rownames(temp)<-c("Hors d'Exploitation", "Bétails Vendu", "Bétail Consommés", "Cuture Vendu", "Culture Consommés")
write.csv(temp, "Word_Outputs/3.Revenu_et_Productivite_des_Menages/5.Prop_VTA_Table.csv")
...
png("Word_Outputs/3.Revenu_et_Productivite_des_Menages/5.Prop_Sources_VTA.png")
par(mfrow=c(1,1),mar=c(8,5,4,8), mgp=c(3,3,1))
barplot(100*Contr_FoodCrops_cons, width=Width,cex.axis=1.5,cex.names=1.2,names.arg =c('Inférieure', 'Moyen\nInférieure', 'Moyen\nSupérieure', 'Haute'), col='green',border=TRUE, ylim=c(0,100), main="Valeur Totale des Activités (%)" , xlab="")
off<-100*Contr_FoodCrops_cons
barplot(100*Contr_FoodCrops_sold, width=Width, cex.axis=1.5,col="blue", offset=off,add=TRUE, border=TRUE)
off<-off+100*Contr_FoodCrops_sold
barplot(100*Contr_Livestock_cons, width=Width, cex.axis=1.5,col="orange", offset=off,add=TRUE, border=TRUE)
...
ylim=c(0,5)
#Barplot of TVA
png("Word_Outputs/3.Revenu_et_Productivite_des_Menages/1.VTA_Graphique.png", height = 800, width = 1000)
par(cex=2)
barplot((as.numeric(dat$valuecropconsumed)/as.numeric(dat$HHsizeMAE)/365)[FSSorder], col="green", ylim=ylim, ylab="Valeur Totale des Activités [$/EHA/Jour]", border=NA)
...
legend("topleft",legend=c("Hors d'Exploitation","Bétails Vendus", "Bétails Consommer", "Cultures Vendus", "Cultures Consommer", paste(FS.p,"% sous la ligne de Pauvreté")), fill=c("black", "orange", "red", "blue", "green",NA))
title(paste0("Valeur_Totale_des_Activités\n", Country, " ", Year), cex.main=1.5)
dev.off()
...
ylim=c(0,5)
#Barplot of FoodAvailability
png("Word_Outputs/3.Revenu_et_Productivite_des_Menages/2.Graphique_Revenu.png", height = 800, width = 1000)
par(cex=2)
income_crop=(as.numeric(dat$cropsales))/as.numeric(dat$HHsizeMAE)/365
barplot(income_crop[FSSorder], col="blue", ylim=ylim, ylab="Revenu ($/EHA/hour)", border=NA)
...
legend("topleft",legend=c("Hors d'Exploitation","Bétail Vendus", "Cultures Vendu",paste(FS.p,"% sous la ligne de la Pauvreté"), paste(FS.p2,"% sans revenu")), fill=c("black", "orange", "blue",NA,NA))
title(paste0("Revenus\n", Country, " ", Year), cex.main=1.5)
dev.off()
...
colnames(temp)<-levels(dat$TVA_quartiles)
rownames(temp)<-gsub("Contr_","",rownames(temp))
rownames(temp)<-c("Hors d'Exploitation", "Bétails Vendu", "Cuture Vendu")
write.csv(temp, "Word_Outputs/3.Revenu_et_Productivite_des_Menages/7.Prop_Cash_Income_Table.csv")
...
png("Word_Outputs/3.Revenu_et_Productivite_des_Menages/7.Prop_Sources_Revenus.png")
par(mfrow=c(1,1),mar=c(8,5,4,8), mgp=c(3,3,1))
barplot(100*Contr_FoodCrops_cons, width=Width,cex.axis=1.5,cex.names=1.2,names.arg =c('Inférieure', 'Moyen\nInférieure', 'Moyen\nSupérieure', 'Haute'), col='green',border=TRUE, ylim=c(0,100), main="Valeurs Totales des Activités (%)" , xlab="")
off<-100*Contr_FoodCrops_cons
barplot(100*Contr_FoodCrops_sold, width=Width, cex.axis=1.5,cex.names=1.2,col="blue",names.arg =c('Inférieure', 'Moyen\nInférieure', 'Moyen\nSupérieure', 'Haute'), ylim=c(0,100),main="Cash Income (PPP$)  %" , xlab="", border=TRUE)
...
colnames(Population_by_Quartile) <- c("Quartiles", 
                                   "Médian Valeurs Totales des Activités ($/personne/jour)", 
                                   "Médian Food Availability  (kcal/personne/jour)") 
# whole_pop<- c("All", 
...
write.csv(Population_by_Quartile, "Word_Outputs/3.Revenu_et_Productivite_des_Menages/4.Population_par_Quartile.csv", row.names = F)
...
village_summary<-data.frame("Village"=village_summary$Village, "Nombre_de_Ménages"=village_summary$Number_of_HHs)
write.csv(village_summary, "Word_Outputs/3.Revenu_et_Productivite_des_Menages/8.Resume_Village.csv")

dat$TVA_quartiles<-factor(dat$TVA_quartiles, levels=c('inférieure', 'moyen inférieure', 'moyen supérieure', 'haute'))

png("Word_Outputs/3.Revenu_et_Productivite_des_Menages/9.Village_Quartile_Résumé.png")
par(mar=c(10,6,3,1))
barplot(table(dat$TVA_quartiles, dat$Village), 
        las=2, 
        main="Quartiles de Richesse par Village", 
...
write.csv(table(dat$TVA_quartiles, dat$Village), "Word_Outputs/3.Revenu_et_Productivite_des_Menages/9.Quartile_de_Richesse_par_Village.csv")
#------
#----------------

png("Word_Outputs/3.Revenu_et_Productivite_des_Menages/10.Revenu_par_Quartile.png")
...
boxplot(list((dat$total_income[dat$TVA_quartiles==levels(dat$TVA_quartiles)[1]]/dat$HHsizemembers[dat$TVA_quartiles==levels(dat$TVA_quartiles)[1]]/365), 
        ...
        outline = F,  col = "lightgray", ylab="int PPP$", main="Revenu en $ par personne par jour", names=c("Inférieure","Moyen Inférieure","Moyen Supérieure","Haute"), las=2) 
dev.off()
...
colnames(Incomes_By_Quartile) <- c("Quartiles", 
                                      "Médian Revenu ($/person/day)", 
                                      "Gamme Interquartile Revenu ($/personne/jour)") 
# whole_pop<- c("All", 
...
write.csv(Incomes_By_Quartile, "Word_Outputs/3.Revenu_et_Productivite_des_Menages/10.Revenu_par_Quartile.csv", row.names = F)
#---------------------------------------------------------


#### Average cash incomes from different sources ($ per household per year)####

png("Word_Outputs/3.Revenu_et_Productivite_des_Menages/11.Resume_des_Revenu.png")
...
boxplot(list((dat$cropsales[dat$TVA_quartiles==levels(dat$TVA_quartiles)[1]]), 
             ...
             #(dat$cropsales)), 
        outline = F,  col = "lightgray",  main="Revenu de Cultures (int PPP$)", names=c("Inf","Moy-Inf","Moy-Sup","Haute"), las=2) 
...
boxplot(list((dat$livestockprodsales[dat$TVA_quartiles==levels(dat$TVA_quartiles)[1]]), 
        ...
        outline = F,  col = "lightgray",  main="Revenu de Bétail (int PPP$)", names=c("Inf","Moy-Inf","Moy-Sup","Haute"), las=2) 
...
boxplot(list((dat$offfarm_income[dat$TVA_quartiles==levels(dat$TVA_quartiles)[1]]), 
             ...
             #(dat$offfarm_income)), 
        outline = F,  col = "lightgray",  main="Revenu Hors d'Exploitation (int PPP$)", names=c("Inf","Moy-Inf","Moy-Sup","Haute"), las=2) 
...
#              (dat$NTFP_Income)), 
#         outline = F,  col = "lightgray",  main="NTFP Income (int PPP$)", names=c("Inf","Moy-Inf","Moy-Sup","Haute", "all"), las=2) 
dev.off()
...
colnames(Income_Break_down) <- c("Quartiles", 
                                 "Vente des Cultures (Median)", 
                                 "Vente des Cultures (Gamme Interquartile)",
                                 "Vente de Bétail (Median)", 
                                 "Vente de Bétail (Gamme Interquartile)",
                                 "Revenu Hors d'Exploitation (Median)", 
                                 "Revenu Hors d'Exploitation (Gamme Interquartile)"
                                 #"NTFP Sales (Median)", 
                                 #"NTFP Sales (Gamme Interquartile)") 
)
# whole_pop<- c("All", 
...
colnames(Income_Break_down)<- c('inférieure', 'moyen inférieure', 'moyen supérieure', 'haute')
Income_Break_down<-Income_Break_down[-which(row.names(Income_Break_down)=="Quartiles"),]
Income_Break_down<-Income_Break_down[-grep("Gamme Interquartile",row.names(Income_Break_down)),]
...
write.csv(Income_Break_down, "Word_Outputs/3.Revenu_et_Productivite_des_Menages/11.Resume_des_Revenus.csv")
...
colnames(Food_Availability_Break_down) <- c("Quartiles", 
                                   "Vente des Cultures (Median)", 
                                   "Vente des Cultures (Gamme Interquartile)",
                                   "Consommation des Cultures (Median)", 
                                   "Consommation des Cultures (Gamme Interquartile)",
                                   "Vente de Bétails (Median)", 
                                   "Vente de Bétails (Gamme Interquartile)",
                                 "Consommation de Bétails (Median)", 
                                 "Consommation de Bétails (Gamme Interquartile)", 
                                 "Revenu Hors d'Exploitation (Median)", 
                                 "Revenu Hors d'Exploitation (Gamme Interquartile)")
                                 # "NTFP Sales (Median)", 
                                 # "NTFP Sales (Gamme Interquartile)", 
                                 # "NTFP Consumption (Median)", 
                                 # "NTFP Consumption (Gamme Interquartile)") 
...
colnames(Food_Availability_Break_down)<- c('inférieure', 'moyen inférieure', 'moyen supérieure', 'haute')
Food_Availability_Break_down<-Food_Availability_Break_down[-which(row.names(Food_Availability_Break_down)=="Quartiles"),]
Food_Availability_Break_down<-Food_Availability_Break_down[-grep("Gamme Interquartile",row.names(Food_Availability_Break_down)),]
...
write.csv(Food_Availability_Break_down, "Word_Outputs/3.Revenu_et_Productivite_des_Menages/12.Resume_Disponibilite_Alimentaire.csv")
...
#####---------
png("Word_Outputs/3.Revenu_et_Productivite_des_Menages/13.Graphique_Revenu_par_Quartile.png")
...
boxplot(list((dat$total_income[dat$TVA_quartiles==levels(dat$TVA_quartiles)[1]]), 
        ...
        outline = F,  col = "lightgray", ylab="int PPP$", main="Revenu en PPP$ par Ménage par An", names=c('Inférieure', 'Moyen Inférieure', 'Moyen Supérieure', 'Haute'), las=2) 
dev.off()
...
write.csv(Total_Income_by_Quartile, "Word_Outputs/3.Revenu_et_Productivite_des_Menages/13.Revenu_par_Quartile.csv", row.names = F)
